...
import { readTrajectory, TYPE_LABEL } from './queries.js'
...
/**
 * Render the card a backfilled session thread opens onto.
 *
 * A thread with nothing in it is worse than no thread: it says a conversation
 * exists and then refuses to say anything about it. This is what makes an
 * opened thread worth opening — what the session was last asked, when, whether
 * it is running, and the buttons to act on it without typing anything.
 *
 * The tail read is why this is bounded by `sessionThreadsBackfill`: it is the
 * one genuinely expensive call in the whole feature, and it is spent once per
 * thread, at creation. A log that will not open costs the excerpt and nothing
 * else — the card still names the session and still carries its controls.
 *
 ...
 */
export async function renderSessionCard(ctx, session, config, t = EN) {
  let excerpt
  try {
    const { entries } = await readTrajectory(ctx, session.id, { limit: 4 })
    const lines = entries
      .filter((entry) => (entry.text ?? '').trim().length > 0)
      .map((entry) => {
        const label = TYPE_LABEL[entry.type] === undefined ? `· ${entry.type}` : t(TYPE_LABEL[entry.type])
        return `${label} ${clip(sanitize(entry.text.replace(/\s*\n\s*/g, ' ')), 200)}`
      })
    if (lines.length > 0) excerpt = lines.join('\n')
  } catch {
    // A log that will not open is a missing excerpt, not a missing card.
  }

  const embed = new EmbedBuilder()
    .setColor(session.live ? COLOR_OK : 0x99aab5)
    .setTitle(`${session.live ? '🟢' : '⚪'} ${clip(sanitize(session.title ?? t('sessions.untitled')), 200)}`)
    .setDescription(excerpt ?? t('thread.noExcerpt'))
    .setFooter({ text: t('thread.cardFooter', { short: session.short, when: when(session.createdAt) }) })

  return {
    embeds: [embed],
    components: actionButtons(session.short, { allowRun: config.allowRun, done: !session.live, t }),
  }
}
...
export function renderModel(selection, t = EN) {
  const { current, models, providers, session } = selection

  // Marked against the session's own model when there is one, because that is
  // the line the reader is checking: which of these am I talking to right now.
  const running = session?.current ?? current

  const list = models.length === 0
    ? t('model.noCatalog')
    : models
      .map((model) => `${model.id === running.model ? '▸ **' : '  '}\`${model.id}\`${model.id === running.model ? '**' : ''} ${clip(sanitize(model.description ?? model.name), 60)}`)
      .join('\n')
  ...
  // Only worth a line when it disagrees with the default; when they match, the
  // description above has already said it.
  if (session !== undefined && `${running.provider}/${running.model}` !== `${current.provider}/${current.model}`) {
    embed.addFields({
      name: t('model.sessionRunning', { short: session.short }),
      value: t('model.sessionRunningBody', { model: `${running.provider}/${running.model}` }),
    })
  }
...
export function renderModelSwitched(change, t = EN) {
  const embed = new EmbedBuilder()
    .setColor(COLOR_OK)
    .setTitle(t(change.scope === 'session' ? 'model.changedSession' : 'model.changed'))
    ...
    .setFooter({
      text: change.scope === 'session'
        ? t('model.scopeSession', { short: change.short })
        : t('model.changedFooter'),
    })
...
export function renderPermissionSwitched(change, t = EN) {
  const embed = new EmbedBuilder()
    ...
    .setFooter({
      text: change.scope === 'session'
        ? t(change.andDefault === true ? 'permission.scopeSessionAndDefault' : 'permission.scopeSession', { short: change.short })
        : t('permission.scopeDefault'),
    })
