 ...
 */
async function settingOptions({ ctx, setting, t }) {
  try {
    if (setting === 'model') {
      const selection = await readModelSelection(ctx)
      return {
        current: selection.current.model,
    ...
    const permissions = await readPermissionPresets(ctx)
    return {
...
export async function buildMenu({ ctx, config, workspace, state, notices = [], mapped = 0, t }) {
  ...
  if (current.setting !== undefined) {
    const picker = await settingOptions({ ctx, setting: current.setting, t })
...
export async function applyMenu({ interaction, ctx, config, workspace, mapped = 0, t }) {
  ...
  if (kind === 'view') state.view = VIEWS.includes(value) ? value : 'sessions'
    ...
    // Same gate as the slash commands: what a later turn may do is not a
    // read, and the model switch is the one that changes no permission.
    if (!config.allowRun && setting !== 'model') throw new TranslatableError('error.writeDisabled')

    if (setting === 'model') notices.push(...render.renderModelSwitched(await switchModel(ctx, value), t).embeds)
    else if (setting === 'preset') notices.push(...render.renderPresetSwitched(await switchAgentPreset(ctx, value), t).embeds)
    else notices.push(...render.renderPermissionSwitched(await switchPermissionPreset(ctx, value), t).embeds)
  }
