 ...
 */
function require_(ctx, name) {
  const service = ctx.get(name)
  if (service === undefined) throw new Error(`the \`${name}\` service is not mounted in this dsh profile`)
  return service
}
...
export async function listWorkspaceSessions(ctx, workspace, limit, { titles: wantTitles = true } = {}) {
  const sessionQuery = require_(ctx, 'sessionQuery')
...
export async function listSubagents(ctx, sessionId, { deep = false }) {
  const subagents = require_(ctx, 'subagents')
  const entries = deep
...
export async function readModelSelection(ctx) {
  const defaults = require_(ctx, 'agentDefaultModel')
...
export async function switchModel(ctx, spec) {
  ...
  if (wanted.length === 0) throw new Error('no model given')

  const slash = wanted.indexOf('/')
  const provider = slash === -1 ? before.provider : wanted.slice(0, slash)
  const model = slash === -1 ? wanted : wanted.slice(slash + 1)
  if (model.length === 0) throw new Error(`\`${wanted}\` names a provider but no model`)
  ...
  if (after.provider !== provider || after.model !== model) {
    throw new Error('the harness accepted no model change — this dsh profile has no settings provider mounted, so the composed default stands')
  }
...
export async function createWorkspace(ctx, path) {
  const registry = require_(ctx, 'workspaceRegistry')

  const wanted = path.trim()
  if (wanted.length === 0) throw new Error('no path given')
  if (!wanted.startsWith('/') && !wanted.startsWith('~')) {
    throw new Error(`\`${wanted}\` is not an absolute path — the harness resolves nothing relative to Discord`)
  }
...
export async function resolveSessionId(ctx, workspace, input) {
  ...
  if (wanted.length === 0) throw new Error('no session given')
  ...
  if (matches.length > 1) throw new Error(`\`${wanted}\` matches ${matches.length} sessions in this workspace — use a longer prefix`)
  throw new Error(`no session in this workspace matches \`${wanted}\``)
}
...
export async function newestSessionId(ctx, workspace) {
  ...
  if (sessions.length === 0) throw new Error('this workspace has no sessions yet')
