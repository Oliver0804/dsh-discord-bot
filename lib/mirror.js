...
import { threadName, THREAD_ARCHIVE_MINUTES } from './topology.js'
...
export function createMirror({ ctx, config, logger, client, runs, resolver, activity }) {
  ...
  async function post(sessionId, state) {
    ...
    if (state.message === undefined) state.message = await sendReviving(placed.channel, payload)
  ...
  /**
   * Send into a channel, reopening it first if it is a thread that archived.
   *
   * A turn quiet for longer than the archive window — waiting on an approval,
   * or on a long tool — comes back to a thread Discord has folded away, and the
   * closing frame would be the message that silently never arrived. Unarchiving
   * costs one call and only on the retry, so the common path is untouched.
   *
   * @param {object} channel - the destination, thread or plain channel.
   * @param {object} payload - the message payload.
   * @returns {Promise<object>} the sent message.
   */
  async function sendReviving(channel, payload) {
    try {
      return await channel.send(payload)
    } catch (error) {
      if (channel.archived !== true || typeof channel.setArchived !== 'function') throw error
  ...
  async function announce(sessionId) {
    ...
    if (located.parentId === undefined) return 'pending'

    const discord = client()
    if (discord === undefined) return 'pending'

    // Deliberately the parent, not the destination: with threads on, this line
    // is the session's entry in the channel's index, and the thread hangs off
    // it. Posting the announcement inside the thread would leave the channel
    // with no sign that the session exists.
    const channel = await discord.channels.fetch(located.parentId).catch(() => null)
    ...
    // The thread is usually already open — `locate` opened it above — and
    // starting one from this message is the nicer shape only when it is not,
    // because Discord renders it as a way in rather than a separate entry.
    if (resolver.threaded && located.channelId === located.parentId) {
      const thread = await message.startThread({
        name: threadName(shortId(sessionId), located.title),
        autoArchiveDuration: THREAD_ARCHIVE_MINUTES,
      }).catch(() => null)
      if (thread !== null) resolver.adopt(sessionId, thread.id)
    }
