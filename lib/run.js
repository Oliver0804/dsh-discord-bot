...
import { NARRATIVE_TYPES } from './queries.js'
 ...
 */
export async function runTurn({ ctx, agent, prompt, report, logger, verbosity = 'minimal', t = EN }) {
  ...
  const body = (status, done) => {
    if (verbosity === 'minimal') {
      if (!done) {
        ...
        return `${status}\n${running}${tools.length > 1 ? `  ·  ${t('run.toolCalls', { count: tools.length })}` : ''}`
      }
