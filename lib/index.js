...
import { createThreadBackfill } from './backfill.js'
...
import { installQuestionMirror } from './questions-mirror.js'
...
import { fileOrphanSessions, listWorkspaces } from './workspaces.js'
...
class DiscordBridge {
  ...
  constructor(ctx, config) {
    ...
    // It takes the client because with `sessionThreads` it does not only read a
    // mapping — it opens the thread a session's messages belong in, and every
    // one of those surfaces has to agree on the answer.
    this.resolver = createChannelResolver({ ctx, config, client: () => this.client })
   ...
   */
  start() {
    // Installed but not yet configured is the normal state for exactly as long
    // as it takes to run the setup script: the bundle layer mounts this row as
    // soon as `dsh plugin add` installs the package, and nothing has told it
    // which guild to bind to yet. Park, and name the fix.
    if (this.config.guildId === undefined) {
      this.logger.warn('dsh-discord-bot: no `guildId` configured — run `npx dsh-discord-bot-setup --profile <name>`, or add a `discord-bot` config override to the profile\'s cordis.patch.yml; staying offline')
      return
    }
    ...
    // Threads for the sessions that already exist. Gated on `mirror` for the
    // same reason the mirror is gated at all: the card it opens each thread
    // with quotes the conversation, and a deployment that has not opted into
    // exporting session content must not have it exported by a sidebar.
    if (this.config.mirror && this.config.sessionThreads) {
      this.backfill = createThreadBackfill({
        ctx: this.ctx,
        config: this.config,
        resolver: this.resolver,
        client: () => this.client,
        logger: this.logger,
        t: ambientTranslator(this.config, () => this.client?.guilds?.cache?.first()?.preferredLocale),
      })
    }
  ...
  async onReady() {
    ...
    //
    // Declining used to end it, which left the phone case unserved — a turn
    // that stops to ask waits for someone at the browser. So a bot that loses
    // the seam goes in through the gateway instead: same questions, same
    // answer entry point, both surfaces live at once.
    if (this.config.answerQuestions && this.disposeQuestions === undefined) {
      ...
      this.disposeQuestions = installQuestionProvider(dependencies) ?? installQuestionMirror(dependencies)
    }
  ...
  async reconcileNow() {
    ...
    // Before reading the workspaces, not after: a session filed here belongs to
    // its workspace in the same pass that maps that workspace to a channel,
    // rather than showing up one sync late.
    const { filed } = await fileOrphanSessions(this.ctx, this.logger)
    ...
    // Fire-and-forget, deliberately after the mapping is adopted and never
    // awaited: opening a thread per existing session is cosmetic work, and a
    // reconcile that waited for it would hold up the mirror and every command
    // behind a tree of log reads.
    this.backfill?.run(workspaces.filter((workspace) => [...result.mapping.values()].includes(workspace.id)))
  ...
  async shutdown() {
    ...
    // Dropped, not awaited: its queue is paced in half-second steps and a
    // teardown must not wait out a tree of them. Each step re-checks the
    // client, which is about to be gone.
