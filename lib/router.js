...
import { applyMenu, closedMenu, isMenuInteraction, openMenu } from './menu.js'
...
export function createRouter({ ctx, config, logger, resync, mappedCount, runs, ownedAgents, activity }) {
  ...
  async function startRun({ workspace, prompt, channelId, post, t, blocks = [] }) {
    ...
    const frame = (text) => ({
      ...
      components: [actionButtons(short, { allowRun: config.allowRun, t })],
    })
  ...
  async function handleAction(interaction) {
    ...
    try {
      ...
      if (action === 'stop') {
        ...
        // The turn is over; retire the stop button but keep the read buttons.
