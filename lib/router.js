...
import { liveSessionFor, preferRunning, resolveAgent, rewindPoints, rewindSession, runTurn, userMessage } from './run.js'
...
import { channelSlug, shortFromThreadName, workspaceIdFromTopic } from './topology.js'
...
export function createRouter({ ctx, config, logger, resync, mappedCount, runs, ownedAgents, activity }) {
  ...
  async function workspaceForChannel(interaction) {
    const workspaceId = workspaceIdFromTopic(await topicOf(interaction))
  ...
  /**
   * The topic that anchors whatever channel something arrived in.
   *
   * A thread carries no topic of its own, so a session thread has to ask its
   * parent — otherwise every command and every message typed inside one reads
   * as an unmapped channel, which is the whole surface `sessionThreads` moves
   * people into. `.parent` is usually cached; the fetch is the cold path.
   *
   * @param {object} source - an interaction or a message.
   * @returns {Promise<string | undefined>} the anchoring topic, when there is one.
   */
  async function topicOf(source) {
    const channel = source.channel
    if (channel?.isThread?.() !== true) return channel?.topic

    const parent = channel.parent
      ?? (channel.parentId === undefined || channel.parentId === null
        ? undefined
        : await source.client?.channels?.fetch(channel.parentId).catch(() => undefined))
    return parent?.topic
  }

  /**
   * The session a thread stands for, when the caller is inside one.
   *
   * This is what removes the guesswork the rest of the routing has to live
   * with. Outside a thread, "which conversation did that mean" is inferred from
   * what is running; inside one, the thread *is* the answer — so a command
   * typed there acts on that session even when three others are live, and a
   * message typed there steers that session rather than whichever one happens
   * to be working.
   *
   * @param {object} source - an interaction or a message.
   * @param {object} workspace - the resolved workspace.
   * @returns {Promise<string | undefined>} the bound session id, when there is one.
   */
  async function boundSessionId(source, workspace) {
    const channel = source.channel
    if (channel?.isThread?.() !== true) return undefined

    const short = shortFromThreadName(channel.name)
    if (short === undefined) return undefined

    // A thread whose session has since been deleted is not an error worth
    // refusing over — the caller falls back to its ordinary targeting.
    return resolveSessionId(ctx, workspace, short).catch(() => undefined)
  }
  ...
  async function targetSession(interaction, workspace) {
    ...
    // Inside a session thread the target is settled — asking for "the newest
    // session in this workspace" there would answer about a different
    // conversation than the one the reader is looking at.
    const id = given === null
      ? (await boundSessionId(interaction, workspace)) ?? await newestSessionId(ctx, workspace)
  ...
  /**
   * The live session a channel's settings change should follow, if any.
   *
   * Everything here is best-effort by design: a channel that maps to no
   * workspace, a workspace with no running session, a deployment with no agents
   * service — each is a reason to change only the default, never a reason to
   * refuse the switch. `/dsh model` has to work from any channel.
   *
   * @param {object} interaction - the Discord interaction.
   * @returns {Promise<string | undefined>} the session to follow, when there is one.
   */
  async function liveSessionInChannel(interaction) {
    try {
      const workspace = await workspaceForChannel(interaction)

      // A thread names its session outright, so there is nothing to infer — but
      // it still has to be running for a switch to reach it, and a bound
      // session that has gone cold degrades to moving the default exactly like
      // a channel with nothing live in it.
      const bound = await boundSessionId(interaction, workspace)
      if (bound !== undefined) return ctx.get('agents')?.get(bound) === undefined ? undefined : bound

      return liveSessionFor(ctx, await listWorkspaceSessions(ctx, workspace, 25), activity)
    } catch {
      return undefined
    }
  }
   ...
   */
  async function startRun({ workspace, prompt, channelId, post, t, blocks = [], inSession }) {
    const all = await listWorkspaceSessions(ctx, workspace, 25)

    // Inside a session thread, that session is the only candidate. Narrowing
    // the list rather than adding a branch keeps `resolveAgent`'s whole
    // contract: a cold one is resumed, and it can never mint a stray session
    // beside the one the thread is named after.
    const bound = inSession === undefined ? undefined : all.filter((entry) => entry.id === inSession)
    const sessions = bound !== undefined && bound.length > 0 ? bound : all
  ...
  async function runWorkspacePrompt(interaction, workspace, prompt, t) {
    await startRun({
      ...
      // `/dsh run` inside a session thread runs in that session, for the same
      // reason typing there does.
      inSession: await boundSessionId(interaction, workspace),
  ...
  async function dispatch(interaction, t) {
    ...
    if (sub === 'sync') {
      ...
      const parts = [
        ...
        result.filed > 0 ? t('sync.filed', { count: result.filed }) : undefined,
    ...
    // `/dsh workspace` is precisely what you run when no channel exists yet, so
    // it runs before a channel has to resolve to one. `/dsh model` reads the
    // channel when there is one and shrugs when there is not, for the same
    // reason: the switch must still work from anywhere.
    if (sub === 'model') {
      const to = interaction.options.getString('to')
      const given = interaction.options.getString('session')

      // Moving the default changes nothing that is already running, which is
      // why this switch has always been ungated. Retargeting a live session
      // steers work in flight, so that half is gated like the rest — and
      // refused here, before the lookup, because a request that cannot be
      // honored has no business first resolving the session it names.
      if (to !== null && given !== null && !config.allowRun) throw new TranslatableError('error.writeDisabled')

      // A named session is the target whether or not it is running — being told
      // it is cold is the answer to that request. Unnamed, the switch follows
      // the conversation in this channel, and a channel with no live one just
      // moves the default.
      const sessionId = given === null
        ? await liveSessionInChannel(interaction)
        : await resolveSessionId(ctx, await workspaceForChannel(interaction), given)

      if (to === null) return render.renderModel(await readModelSelection(ctx, sessionId), t)

      // Ungated and unnamed, the switch degrades to the default alone, and the
      // footer says so rather than implying the conversation moved with it.
      return render.renderModelSwitched(await switchModel(ctx, to, config.allowRun ? sessionId : undefined), t)
    }
    ...
    if (sub === 'permission') {
      ...
      // Naming a session is the harness's own `/permission`: that conversation
      // and nothing else. Not naming one is a question about the work in this
      // channel, so it moves the channel's conversation *and* the default —
      // otherwise the switch changes nothing anyone can see from here.
      return render.renderPermissionSwitched(
        given === null
          ? await switchPermissionPreset(ctx, to, sessionId, { andDefault: true })
          : await switchPermissionPreset(ctx, to, sessionId),
        t,
      )
    }
  ...
  async function handleMessage(message) {
    ...
    try {
      await startRun({
        ...
        // Typing inside a session's thread is talking to that session, which is
        // the point of having threads at all.
        inSession: await boundSessionId(message, workspace),
