...
export const commands = [
  describe(new SlashCommandBuilder().setName('dsh'), 'cmd.root')
    ...
    .addSubcommand((sub) => describe(sub.setName('menu'), 'cmd.menu'))
    .addSubcommand((sub) => describe(sub.setName('sessions'), 'cmd.sessions')
      .addIntegerOption((option) => limit(option, 'opt.limitSessions')))
    .addSubcommand((sub) => describe(sub.setName('search'), 'cmd.search')
      .addStringOption((option) => describe(option.setName('query'), 'opt.query')
        .setRequired(true)
        .setMaxLength(200))
      .addIntegerOption((option) => limit(option, 'opt.limitSearch')))
    ...
    .addSubcommand((sub) => describe(sub.setName('model'), 'cmd.model')
      .addStringOption((option) => describe(option.setName('to'), 'opt.to')
        .setAutocomplete(true)
        .setRequired(false)))
    .addSubcommand((sub) => describe(sub.setName('todos'), 'cmd.todos')
      .addStringOption(session))
    .addSubcommand((sub) => describe(sub.setName('stop'), 'cmd.stop')
      .addStringOption(session))
    .addSubcommand((sub) => describe(sub.setName('rewind'), 'cmd.rewind')
      .addStringOption(session))
    .addSubcommand((sub) => describe(sub.setName('context'), 'cmd.context')
      .addStringOption(session))
    .addSubcommand((sub) => describe(sub.setName('export'), 'cmd.export')
      .addStringOption(session))
    .addSubcommand((sub) => describe(sub.setName('cmd'), 'cmd.cmd')
      .addStringOption((option) => describe(option.setName('name'), 'opt.cmdName')
        .setAutocomplete(true)
        .setRequired(false))
      .addStringOption((option) => describe(option.setName('input'), 'opt.cmdInput')
        .setRequired(false)
        .setMaxLength(1000)))
    .addSubcommand((sub) => describe(sub.setName('preset'), 'cmd.preset')
      .addStringOption((option) => describe(option.setName('to'), 'opt.presetTo')
        .setAutocomplete(true)
        .setRequired(false)))
    .addSubcommand((sub) => describe(sub.setName('permission'), 'cmd.permission')
      .addStringOption((option) => describe(option.setName('to'), 'opt.permissionTo')
        .setAutocomplete(true)
        .setRequired(false))
      .addStringOption((option) => describe(option.setName('session'), 'opt.sessionLive')
        .setAutocomplete(true)
        .setRequired(false)))
