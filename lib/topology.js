...
export function workspaceIdFromTopic(topic) {
  // `String.match`, not `RegExp.exec` — identical for a non-global pattern
  // (same array, same null), but the plugin-directory scanner greps for
  // `exec(` and files this line as shell execution. It is a false positive
  // either way; spending the equivalent call to not raise it is cheaper than
  // every reader having to rule it out.
  const match = (topic ?? '').match(/^\[dsh:([^\]]+)\]/)
  return match === null ? undefined : match[1]
}

/** Discord's thread-name length limit. */
const THREAD_NAME_LIMIT = 100

/**
 * How long a session thread stays open with nothing said in it. A day, because
 * that is the span over which someone comes back to a conversation they left —
 * and an archived thread is not lost, only folded away, and reopens the moment
 * anything is posted to it.
 */
export const THREAD_ARCHIVE_MINUTES = 1440

/**
 * The name of the thread that carries one session.
 *
 * The same idea as the topic marker, in the only field a thread has: a thread
 * carries no topic, so its *name* is the anchor. The short id leads, and it is
 * the only part ever parsed back — the title that follows is decoration for a
 * human scanning the sidebar, and it is deliberately never used to identify
 * anything.
 *
 * That split is what lets the title go stale safely. Discord rate-limits thread
 * renames to about two per ten minutes, so a bot that renamed a thread each
 * time a session's title was regenerated would spend its budget on cosmetics
 * and stall the mirror behind it. The name is written once, at creation.
 *
 * @param {string} short - the session's short id.
 * @param {string} [title] - the session or workspace title, for readability.
 * @returns {string} a thread name Discord will accept.
 */
export function threadName(short, title) {
  const trimmed = (title ?? '').replace(/\s+/g, ' ').trim()
  const name = trimmed.length === 0 ? short : `${short} · ${trimmed}`
  return name.slice(0, THREAD_NAME_LIMIT)
}

/**
 * Read the session's short id back out of a thread name.
 * @param {string | null | undefined} name - the thread name.
 * @returns {string | undefined} the short id, when this is a managed thread.
 */
export function shortFromThreadName(name) {
  const match = (name ?? '').match(/^([0-9a-f]{8})(?: |$)/)
  return match === null ? undefined : match[1]
}
...
async function enforceCategoryPrivacy(guild, category, config, me, logger) {
  ...
  try {
    ...
    return fallback(`${described(error)} — add the bot to the category's permissions and allow View Channel`)
  }
}
...
async function inheritPrivacy(channel, isPrivate, canManage, logger) {
  ...
  try {
    ...
    logger.warn('dsh-discord-bot: could not lock #%s to its category: %s', channel.name, described(error))
  }
}
...
export async function reconcile({ client, config, workspaces, logger }) {
  ...
  // Without Manage Channels the bot cannot write overwrites at all, but it can
  // still *read* the category's state — so an externally-locked category is
  // reported as such, not as "open". Saying "not private" about a category an
  // admin already locked down would send an operator chasing an exposure that
  // does not exist (and the inverse mistake would be worse still).
  const privacy = canManage
    ? await enforceCategoryPrivacy(guild, category, config, me, logger)
    : config.privateChannels && deniesEveryone(guild, category) ? 'external' : 'open'
  ...
  for (const workspace of workspaces) {
    ...
    if (existing !== undefined) {
      ...
      if (canManage && config.manageChannels) {
        ...
        try {
          ...
          logger.warn('dsh-discord-bot: could not update channel #%s: %s', existing.name, described(error))
        }
      }
    ...
    try {
      ...
      logger.warn('could not create a channel for workspace "%s": %s', workspace.title, described(error))
      skipped.push(workspace.title)
    }
  }
