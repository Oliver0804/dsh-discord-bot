...
export const ANSWER_TIMEOUT_MS = 900_000
/** Discord's ceiling on select options. */
const OPTION_LIMIT = 25

/**
 * Put one question to a channel and wait for someone to answer it.
 *
 * Shared by both surfaces: the seam provider, which owns the ask outright, and
 * the apiproxy mirror, which does not. The card is identical either way —
 * only what the caller does with a timeout differs, which is why the expiry
 * notice is a parameter rather than a constant.
 *
 * The answer is shaped the way the harness's own schema wants it: a chosen
 * option travels in `selected`, free text in `custom`. They are not
 * interchangeable — `selected` is validated against the question's option
 * labels, so typed text placed there is rejected as an illegal choice.
 *
 * @param {object} args - question inputs.
 * @param {object} args.channel - the Discord channel to ask in.
 * @param {string} args.sessionId - the session the ask belongs to.
 * @param {object} args.question - one question from the request.
 * @param {number} args.position - 1-based index within the request.
 * @param {number} args.total - how many questions the request carries.
 * @param {AbortSignal} [args.signal] - aborts when the asker withdraws.
 * @param {object} args.config - the validated plugin config.
 * @param {(key: string, params?: object) => string} args.t - translator.
 * @param {string} [args.expiryNotice] - i18n key for the unanswered card.
 * @param {string} [args.withdrawnNotice] - i18n key for a card retracted by the asker.
 * @returns {Promise<{id: string, selected: string[], custom?: string}>} the answer item.
 */
export async function askOne({ channel, sessionId, question, position, total, signal, config, t, expiryNotice = 'question.timedOut', withdrawnNotice = 'question.withdrawn' }) {
  ...
  const chosen = await new Promise((resolve, reject) => {
    ...
    collector.on('end', (_collected, reason) => {
      ...
      modalResolvers.delete(modalId)
      if (reason === 'answered' || reason === 'retracted') return

      void card.edit({
        embeds: [EmbedBuilder.from(embed).setColor(0x99aab5).setFooter({ text: t(reason === 'aborted' ? withdrawnNotice : expiryNotice) })],
        components: [],
      }).catch(() => {})

      reject(Object.assign(new Error(reason === 'aborted' ? 'the asker withdrew the question' : 'nobody answered in time'), { code: 'ASK_CANCELLED' }))
    })
  })

  return { id: question.id, ...chosen }
}
