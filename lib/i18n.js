...
const STRINGS = {
  en: {
    ...
    'status.pricing': 'billing period',
    'status.peak': '🔴 peak — half price after {until} (Beijing)',
    'status.offPeak': '🟢 off-peak — doubles at {until} (Beijing)',

    // Model
    // Session threads
    'thread.noExcerpt': '_nothing said in this session yet — whatever it does from now on appears here_',
    'thread.cardFooter': 'session {short} · started {when} · this thread now carries everything it does',

    'model.title': '🧠 default model',
    'model.current': 'New sessions start on **{provider}/{model}**{effort}.\nA running session keeps the model it last ran on; switching from its channel moves that one too.',
    ...
    'model.changedSession': '🧠 session model changed',
    'model.changedBody': '`{before}` → **{after}**',
    'model.changedFooter': 'applies to sessions created from now on; no running session here to move',
    'model.scopeSession': 'session {short} moves at its next turn, and new sessions start here too',
    'model.sessionRunning': 'session {short} is running on',
    'model.sessionRunningBody': '**{model}** — a session keeps the model it last ran on until it is switched',
    ...
    'sync.filed': '{count} session(s) filed under their workspace',
    ...
    'error.modelNotApplied': 'the default is now **{model}**, but session {short} did not take it: {reason}',
    ...
    'error.sessionNotLive': 'session {short} is not running, so its own settings cannot be changed — only the default for new sessions can',
    ...
    'question.answeredElsewhere': 'answered in the web UI',
    'question.expiredElsewhere': 'this card expired — the question is still waiting in the web UI',
    ...
    'preset.current': 'New sessions are composed from **{id}**.\nA session that has taken one turn has its preset fixed by the harness — unlike the model, this one cannot be switched from here or anywhere else.',
    'preset.roster': 'available presets',
    'preset.broken': '_unusable — {reason}_',
    'preset.changed': '🧩 agent preset changed',
    'preset.footer': 'applies to sessions created from now on — a session that has started keeps its preset, and no switch anywhere can move it',
    ...
    'permission.scopeDefault': 'applies to sessions created from now on; no running session here to move',
    'permission.scopeSession': 'applies to session {short} from its next tool call',
    'permission.scopeSessionAndDefault': 'applies to session {short} from its next tool call, and to new sessions too',
    ...
    // A range, because the totals carry no time buckets: off-peak to peak.
    'stats.cost': 'est. CN¥{amount}',
  ...
  'zh-Hant': {
    ...
    'status.pricing': '計費時段',
    'status.peak': '🔴 高峰 —— {until}（北京）後半價',
    'status.offPeak': '🟢 空閒 —— {until}（北京）起漲一倍',

    // Session threads
    'thread.noExcerpt': '_這個工作階段還沒說過話 —— 從現在起它做的事都會出現在這裡_',
    'thread.cardFooter': '工作階段 {short} · 開始於 {when} · 之後它做的每件事都在這條討論串裡',

    'model.title': '🧠 預設模型',
    'model.current': '新工作階段會使用 **{provider}/{model}**{effort}。\n運行中的工作階段維持它上一輪實際跑的模型；從它的頻道切換會一併帶著它換。',
    ...
    'model.changedSession': '🧠 工作階段模型已變更',
    'model.changedBody': '`{before}` → **{after}**',
    'model.changedFooter': '從現在開始建立的工作階段適用；這裡沒有運行中的工作階段可以一併變更',
    'model.scopeSession': '工作階段 {short} 從下一輪開始生效，新建的工作階段也會用這個',
    'model.sessionRunning': '工作階段 {short} 正在使用',
    'model.sessionRunningBody': '**{model}** —— 工作階段會維持它上一輪跑的模型，直到被切換為止',
    ...
    'sync.filed': '{count} 個工作階段已歸入所屬工作區',
    ...
    'error.modelNotApplied': '預設已改為 **{model}**，但工作階段 {short} 沒有套用成功：{reason}',
    'error.noPath': '沒有指定路徑',
    ...
    'error.sessionNotLive': '工作階段 {short} 沒有在執行，因此無法改它自己的設定 —— 只能改新工作階段的預設值',
    ...
    'question.answeredElsewhere': '已在網頁介面回答',
    'question.expiredElsewhere': '這張卡片已失效 —— 問題還在網頁介面等你回答',
    ...
    'preset.current': '新工作階段會以 **{id}** 組成。\n跑過一輪的工作階段，它的 preset 已被 harness 固定 —— 與模型不同，這個從這裡或任何地方都切換不了。',
    'preset.roster': '可用的預設',
    'preset.broken': '_無法使用 —— {reason}_',
    'preset.changed': '🧩 Agent 預設已變更',
    'preset.footer': '對此後新建的工作階段生效 —— 已經開始的工作階段維持原本的 preset，任何地方的切換都改不動它',
    ...
    'permission.scopeDefault': '對此後新建的工作階段生效；這裡沒有運行中的工作階段可以一併變更',
    'permission.scopeSession': '從工作階段 {short} 的下一次工具呼叫起生效',
    'permission.scopeSessionAndDefault': '從工作階段 {short} 的下一次工具呼叫起生效，新建的工作階段也適用',
  ...
  'zh-Hans': {
    ...
    'status.pricing': '计费时段',
    'status.peak': '🔴 高峰 —— {until}（北京）后半价',
    'status.offPeak': '🟢 空闲 —— {until}（北京）起涨一倍',

    // Session threads
    'thread.noExcerpt': '_这个会话还没说过话 —— 从现在起它做的事都会出现在这里_',
    'thread.cardFooter': '会话 {short} · 开始于 {when} · 之后它做的每件事都在这条讨论串里',

    'model.title': '🧠 默认模型',
    'model.current': '新会话会使用 **{provider}/{model}**{effort}。\n运行中的会话保持它上一轮实际跑的模型；从它的频道切换会一并带着它换。',
    'model.effort': ' · 推理强度 `{effort}`',
    'model.catalog': '{provider} 的模型',
    'model.noCatalog': '_这个 provider 没有公布目录；你仍然可以直接输入确切的 id 来切换_',
    'model.providers': 'providers：{providers}',
    'model.changed': '🧠 默认模型已变更',
    'model.changedSession': '🧠 会话模型已变更',
    'model.changedBody': '`{before}` → **{after}**',
    'model.changedFooter': '从现在开始创建的会话适用；这里没有运行中的会话可以一并变更',
    'model.scopeSession': '会话 {short} 从下一轮开始生效，新建的会话也会用这个',
    'model.sessionRunning': '会话 {short} 正在使用',
    'model.sessionRunningBody': '**{model}** —— 会话会保持它上一轮跑的模型，直到被切换为止',
    ...
    'sync.filed': '{count} 个工作阶段已归入所属工作区',
    ...
    'error.modelNotApplied': '默认已改为 **{model}**，但会话 {short} 没有应用成功：{reason}',
    'error.noPath': '没有指定路径',
    ...
    'error.sessionNotLive': '会话 {short} 没有在运行，因此无法改它自己的设置 —— 只能改新会话的默认值',
    ...
    'question.answeredElsewhere': '已在网页界面回答',
    'question.expiredElsewhere': '这张卡片已失效 —— 问题还在网页界面等你回答',
    ...
    'preset.current': '新会话会以 **{id}** 组成。\n跑过一轮的会话，它的 preset 已被 harness 固定 —— 与模型不同，这个从这里或任何地方都切换不了。',
    'preset.roster': '可用的预设',
    'preset.broken': '_无法使用 —— {reason}_',
    'preset.changed': '🧩 Agent 预设已变更',
    'preset.footer': '对此后新建的会话生效 —— 已经开始的会话保持原本的 preset，任何地方的切换都改不动它',
    ...
    'permission.scopeDefault': '对此后新建的会话生效；这里没有运行中的会话可以一并变更',
    'permission.scopeSession': '从会话 {short} 的下一次工具调用起生效',
    'permission.scopeSessionAndDefault': '从会话 {short} 的下一次工具调用起生效，新建的会话也适用',
